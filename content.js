// Load EasyList selectors (IDs only) from the file — assumed preloaded
...
function isElementHidden(el) {
  ...
  function matchesEasylist(el) {
    if (el.id && easylistSelectors.has(el.id)) {
      return true;
    }
    for (const cls of el.classList) {
      if (easylistSelectors.has(cls)) {
        return true;
      }
    }
    return false;
  ...
  function detectInitialHiddenElements() {
    const all = document.querySelectorAll('*');
    all.forEach(el => {
      if (isElementHidden(el) && matchesEasylist(el)) {
        console.log('Hadi [EasylistMatch] Hidden element:', el);
      }
    });
  }
      ...
      for (const mutation of mutations) {
        mutation.addedNodes.forEach(node => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            const el = node;
            if (isElementHidden(el) && matchesEasylist(el)) {
              console.log('Hadi [EasylistMatch] Hidden added element:', el);
            }
            el.querySelectorAll?.('*').forEach(child => {
              if (isElementHidden(child) && matchesEasylist(child)) {
                console.log('Hadi [EasylistMatch] Hidden added descendant:', child);
              }
            });
          }
        });
  ...
  window.addEventListener('load', () => {
    console.log('Hadi [HiddenDetector] Page loaded. Scanning...');
    detectInitialHiddenElements();
    observeMutations();
  });